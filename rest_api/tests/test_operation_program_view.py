...
class OperationProgramViewSetTest(BaseTestCase):
    ...
    def test_create_with_group_permissions(self):
        ...
        response = self.client.post('/api/operation-programs/', data={
            "start_at": "2021-01-01",
    ...
    def test_update_with_group_permissions(self):
        self.login_op_user()
        date = "2021-01-01"
        self.client.post('/api/operation-programs/', data={
            "start_at": date,
            "op_type": "/api/operation-program-types/1/"})
        op = OperationProgram.objects.get(start_at=date)
        response = self.client.put(f'/api/operation-programs/{op.id}/', data={
            "start_at": "2021-01-02",
            "op_type": "/api/operation-program-types/1/"})
        self.assertEqual(response.status_code, 200)
